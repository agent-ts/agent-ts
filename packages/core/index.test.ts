...
import { makeAgent, TEXT_CHUNK, TOOL_CALL_REQUEST, TOOL_CALL_RESPONSE, NEW_ITERATION } from './index';
...
describe('makeAgent', () => {
  test('应该正确处理完整的对话流程', async () => {
    ...
    // 验证完整的流程
    const iterations = chunks.filter(chunk => chunk.type === NEW_ITERATION);
    expect(iterations.length).toBe(2); // 应该有两轮迭代

    // 验证工具调用
    const toolCalls = chunks.filter(chunk => chunk.type === TOOL_CALL_REQUEST);
    expect(toolCalls.length).toBe(1);

    // 验证工具响应
    const toolResponses = chunks.filter(chunk => chunk.type === TOOL_CALL_RESPONSE);
    expect(toolResponses.length).toBe(1);

    // 验证最终的文本响应
    const textChunks = chunks
      .filter(chunk => chunk.type === TEXT_CHUNK)
  ...
  test('应该处理多轮工具调用', async () => {
    ...
    const mockOpenAI = makeMockAgent([firstIteration, secondIteration, thirdIteration]);
  ...
  test('应该处理工具调用错误后的恢复', async () => {
    ...
    // 验证错误响应
    const errorResponse = chunks.find(chunk => chunk.type === TOOL_CALL_RESPONSE);
