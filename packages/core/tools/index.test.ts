...
import { makeTool, schema } from './index';
...
describe('makeTool', () => {
  ...
  test('应该正确处理对象参数', async () => {
    const tool = makeTool({
      ...
      call: async (params) => `Hello, ${params.name}! You are ${params.age} years old.`
    });
  ...
  test('应该处理数组参数', async () => {
    ...
    const result = await tool.call(JSON.stringify({ numbers: [1, 2, 3, 4, 5] }));
