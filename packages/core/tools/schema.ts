...
export function parseString<T extends StringSchema>({ enum: enumValues, maxLength, minLength, format }: T, path: string, input: unknown): ValueOf<T> {
  if (typeof input !== 'string') {
    throw new ParseError(`input is not a string: ${input}`, path, input);
  }
  if (typeof maxLength === "number" && input.length > maxLength) {
    throw new ParseError(`string is too long: ${input.length} > ${maxLength}`, path, input);
  }
  if (typeof minLength === "number" && input.length < minLength) {
    throw new ParseError(`string is too short: ${input.length} < ${minLength}`, path, input);
  }
  if (enumValues && !enumValues.includes(input)) {
    throw new ParseError(`string is not in enum ${JSON.stringify(enumValues)}: ${input}`, path, input);
  }
...
export function parseNumber<T extends NumberSchema>({ enum: enumValues, minimum, maximum }: T, path: string, input: unknown): ValueOf<T> {
  if (typeof input !== 'number') {
    throw new ParseError(`input is not a number: ${input}`, path, input);
  }
  if (enumValues && !enumValues.includes(input)) {
    throw new ParseError(`number is not in enum ${JSON.stringify(enumValues)}: ${input}`, path, input);
  }
  if (typeof minimum === "number" && input < minimum) {
    throw new ParseError(`number is too small: ${input} < ${minimum}`, path, input);
  }
  if (typeof maximum === "number" && input > maximum) {
    throw new ParseError(`number is too large: ${input} > ${maximum}`, path, input);
  }
  return input as ValueOf<T>;
}

export function parseInteger<T extends IntegerSchema>({ enum: enumValues, minimum, maximum }: T, path: string, input: unknown): ValueOf<T> {
  if (typeof input !== 'number') {
    throw new ParseError(`input is not a number: ${input}`, path, input);
  }
